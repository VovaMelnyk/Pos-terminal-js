...
class Dishes{
    ...
    createContentHeader(){
        ...
        root.innerHTML = '<div class="content-header"><h2 class="content-header__title">Категории <span class="content-header__quantity" data-action="quantity">0</span></h2><a href=""><button class="content-header__button" data-action="add">Добавить</button></a></div>';

    }
    createFastSearch(){
        const root = document.querySelector('#root');
        root.innerHTML += ' <div class="search"><form class="search-form"><button class="search-form__btn" type="submit"></button><input type="search" class="search-form__input" name="fast-search" placeholder="Быстрый поиск" autofocus></form></div>';
    }
    createList(){
        const root = document.querySelector('#root');
        root.innerHTML += '   <section class="categories"><div class="categories__titles"><h3 class="categories__title">Категория</h3></div><ul class="categories__list"></ul></section>';
    }
        ...
        list.addEventListener('click',(e) =>{
            ...
            const exitByCLick = function (e){
                ...
                if(element.dataset.action === 'hide'){
                    ...
                    element.parentNode.parentNode.parentNode.previousElementSibling.classList.add('low-opacity');
                    return;
                }
                ...
                if(element.dataset.action === 'review'){
                    ...
                    element.parentNode.parentNode.parentNode.previousElementSibling.classList.remove('low-opacity');
                }
